...
(function (mod) {
  ...
  CodeMirror.defineMode("julia", function (_conf, parserConf) {
    ...
    const specialChar = "\\\\[abfnrtv0%?'\"\\\\]";
    const singleChar = "([^\\u0027\\u005C\\uD800-\\uDFFF]|[\\uD800-\\uDFFF][\\uDC00-\\uDFFF])";
    const operators = parserConf.operators || /^\.?[|&^\\%*+\-<>!=\/]=?|\?|~|:|\$|\.[<>]|<<=?|>>>?=?|\.[<>=]=|->?|\/\/|\bin\b(?!\()|[\u2208\u2209](?!\()/;
    const delimiters = parserConf.delimiters || /^[;,()[\]{}]/;
    const identifiers = parserConf.identifiers || /^[_A-Za-z\u00A1-\uFFFF][\w\u00A1-\uFFFF]*!*/;
    const charsList = [octChar, hexChar, specialChar, singleChar];
    ...
    const keywordList = ['if', 'else', 'elseif', 'while', 'for', 'begin', 'let', 'end', 'do', 'try', 'catch', 'finally', 'return', 'break', 'continue', 'global', 'local', 'const', 'export', 'import', 'importall', 'using', 'function', 'macro', 'module', 'baremodule', 'type', 'immutable', 'quote', 'typealias', 'abstract', 'bitstype'];
    const builtinList = ['true', 'false', 'nothing', 'NaN', 'Inf'];

    //var stringPrefixes = new RegExp("^[br]?('|\")")
    const stringPrefixes = /^(`|"{3}|([brv]?"))/;
    const chars = wordRegexp(charsList, "'");
    ...
    function tokenBase(stream, state) {
      ...
      const isDefinition = state.isDefinition ||
    ...
    function callOrDef(stream, state) {
      ...
      if (currentScope(state) == '(' && stream.match(/^\)/)) {
        ...
        if (state.scopes.length <= state.firstParenPos) {
          const isDefinition = stream.match(/^\s*?=(?!=)/, false);
